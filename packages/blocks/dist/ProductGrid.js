...
export function ProductGrid({ apiBaseUrl, apiEndpoint, title, limit = 12, collectionId, useDemoData, tenantId, storeId, titleFontFamily, titleTextColor, titleFontSize, titleFontWeight, titleFontStyle, buttonBackgroundColor, buttonColor, buttonFontFamily, buttonFontSize, buttonFontWeight, buttonFontStyle, buttonBorderRadius, buttonPadding }) {
    ...
    if (products.length === 0) {
        return (_jsxs("section", { className: "block block-product-grid", children: [_jsx("h2", { className: "block-heading", style: Object.keys(titleStyle).length ? titleStyle : undefined, children: isHtml(title) ? _jsx("span", { dangerouslySetInnerHTML: { __html: sanitizeHtml(title) } }) : title }), _jsx("p", { children: useDemoData
                        ? 'No demo products found.'
                        : 'No products found for this tenant/store. Verify Tenant ID, Store ID, and API data.' })] }));
    }
    return (_jsxs("section", { className: "block block-product-grid", children: [_jsx("h2", { className: "block-heading", style: Object.keys(titleStyle).length ? titleStyle : undefined, children: isHtml(title) ? _jsx("span", { dangerouslySetInnerHTML: { __html: sanitizeHtml(title) } }) : title }), _jsx("div", { className: "product-grid", children: products.map((product) => (_jsxs("article", { className: "product-card", children: [product.image && (_jsx("div", { className: "product-image", children: _jsx("img", { src: product.image, alt: product.title, loading: "lazy" }) })), _jsxs("div", { className: "product-info", children: [_jsx("h3", { className: "product-title", children: product.title }), product.description && (_jsx("p", { className: "product-description", children: product.description })), _jsxs("div", { className: "product-price", children: ["$", product.price.toFixed(2)] }), (useDemoData ? product.handle : product.id) && (_jsx("a", { href: `/products/${encodeURIComponent(useDemoData ? product.handle : product.id)}`, className: "product-link", style: Object.keys(linkStyle).length ? linkStyle : undefined, children: "View Product" }))] })] }, product.id))) })] }));
}
